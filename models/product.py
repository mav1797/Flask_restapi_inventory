...
class ProductModel(db.Model):
    ...
    @classmethod
    def find_by_name_and_category(cls,product_name,product_category):
        return cls.query.filter_by(product_name=product_name).filter_by(product_category=product_category).first()

    @classmethod
    def find_by_product_category(cls,product_category):
        return cls.query.filter_by(product_category=product_category)
    # save changes / new data to databse
    def save_to_db(self):
        db.session.add(self)
        db.session.commit()

    # convert any time ro CST time
    @classmethod
